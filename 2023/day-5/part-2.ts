...
const tmp = seedsTxt!.slice(seedsTxt!.indexOf(':') + 1).trim().split(' ').map((it) => parseInt(it))

let seeds: Array<number> = []
for (let idx = 0; idx < tmp.length; idx += 2) {
	console.log(idx, seeds.length)
	const element = tmp[idx]
	const len = tmp[idx + 1]
	seeds = seeds.concat(Array.from(new Array(len), (_, nIdx) => element + nIdx))
}
